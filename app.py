...
# 3. 앱 타이틀 및 소개
st.title("📅 스마트 수행평가 올인원 플래너")
st.markdown("""
스마트 수행평가 도우미 앱에 오신 것을 환영합니다! 🎉  
왼쪽 사이드바 메뉴를 통해 **시간표 확인, D-Day 관리, AI 채점 분석** 기능을 모두 이용할 수 있습니다.
""")

st.info("💡 **이용 가이드:** 왼쪽 메뉴에서 `1_시간표_및_일정` 또는 `2_AI_채점_분석` 탭을 선택해 보세요!")
...
st.subheader("📋 전체 등록된 수행평가 목록 기본 대시보드")
...
if not st.session_state.evaluations.empty:
    ...
    for idx, row in df_sorted.iterrows():
        with st.expander(f"📌 [{row['과목']}] {row['수행평가명']} (마감일: {row['마감일']})"):
